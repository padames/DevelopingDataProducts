...
#           pch=21, bg=alpha("red",0.25), cex=cex_values)
    ...
    # years_string <- (as.character.Date(storm_data_ST_EvtType$BGN_DATE,format="%d/%m/%Y"))
    # years <- as.POSIXlt(years_string,format = "%d/%M/%Y")
    # years <- as.character(format( as.Date(storm_data_ST_EvtType$BGN_DATE, format = "%m/%d/%Y %H:%M:%S"), "%Y"))
    # effectCount <- as.character(storm_data_ST_EvtType[EffectName][,1]) 
